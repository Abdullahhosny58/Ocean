...
const PricingCard: FC<PricingCardProps> = ({
  ...
  return (
    ...
    >
      <figure
        className={`absolute w-72 h-60 bg-sky-600 -z-10 rounded-full 
        blur-3xl opacity-20 -top-12 ${
          glowPosition === "left" ? "-left-8" : "-right-8"
        }`}
      />
      {isMostPopular && (
        <p
          className="absolute top-5 right-6 bg-sky-600
         text-white text-xs font-semibold 
         capitalize py-2 px-3 rounded-3xl"
        >
          Most Popular
        </p>
      )}
